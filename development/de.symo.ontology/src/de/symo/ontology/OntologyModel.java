...
import de.symo.ontology.util.OntologyHelper;
...
public final class OntologyModel {
	...
	private OntologyModel() {
		OntModel newOntologyModel = ModelFactory.createOntologyModel(ProfileRegistry.OWL_DL_LANG);
	...
	public OntClass addOntClass(URI ontClasslassURI) {
		...
		if (getOntologyClass(ontClasslassURI) == null) {
			returnOntClass = ontologyModel.createClass(ontClasslassURI.toString());

		} else {
			System.out.println("Class " + ontClasslassURI + " already existed in the ontology");
		}
	...
	public Individual addIndividual(URI individualURI, OntClass ontClass) {
		...
		if (individual == null) {
			individual = ontologyModel.createIndividual(individualURI.toString(), ontClass);
		} else {

			System.out.println("Individual " + individualURI + " already existed in the ontology");
		}
		return individual;
	}
	 ...
	 */
	public void addObjectPropertyInstance(URI domainIndividualURI, URI objectPropertyURI, URI rangeIndividualURI) {

		if (!containsIndividual(domainIndividualURI)) {
			System.out.println("The domain Individual " + domainIndividualURI + " is missing in the ontology. Can't create new object property instance");
			return;
		}

		if (!containsIndividual(rangeIndividualURI)) {
			System.out.println("The range Individual " + rangeIndividualURI + " is missing in the ontology. Can't create new object property instance");
			return;
		}

		if (!containsProperty(objectPropertyURI)) {
			System.out.println("The ObjectProperty " + objectPropertyURI + " is missing in the ontology. Can't create new object property instance");
			return;
		}

		ObjectProperty objectProperty = getOntologyModel().getObjectProperty(objectPropertyURI.toString());

		Individual targetIndividual = getOntologyModel().getIndividual(rangeIndividualURI.toString());

		if (!getOntologyModel().getIndividual(domainIndividualURI.toString()).hasProperty(objectProperty, targetIndividual)) {
			getOntologyModel().getIndividual(domainIndividualURI.toString()).addProperty(objectProperty, targetIndividual);
			//
	 ...
	 */
	public void addDatatypePropertyInstance(URI domainIndividualURI, URI datatypePropertyURI, String propertyValue) {
		if (!containsIndividual(domainIndividualURI)) {
			System.out.println("The domain Individual " + domainIndividualURI + " is missing in the ontology. Can't create new object property instance");
			return;
		}
		...
		if (!containsProperty(datatypePropertyURI)) {
			System.out.println("The Datatype  " + datatypePropertyURI + " is missing in the ontology. Can't create new object property instance");
			return;
		}

		DatatypeProperty datatypeProperty = getOntologyModel().getDatatypeProperty(datatypePropertyURI.toString());

		if (!getIndividual(domainIndividualURI).hasProperty(datatypeProperty)) {
			getIndividual(domainIndividualURI).addProperty(datatypeProperty, propertyValue);
	...
	public ObjectProperty getObjectProperty(URI objectPropertyName) {
		return getOntologyModel().getObjectProperty(objectPropertyName.toString());
	}
	...
	public List<ObjectProperty> getObjectProperties() {
		List<ObjectProperty> objectProperties = new ArrayList<ObjectProperty>(getOntologyModel().listObjectProperties().toList());
	...
	public List<Individual> getIndividuals() {
		...
		Iterator<Individual> individualsIterator = getOntologyModel().listIndividuals();
	...
	public List<OntClass> getClassesWithInstances(Ontology ontology) {
		List<OntClass> classesInOntology = getResourcesByClass(OntClass.class, ontology);
		List<OntClass> classesWithInstances = new ArrayList<OntClass>();
		for (OntClass ontologyClass : classesInOntology) {
	...
	public List<Ontology> getOntologiesURIs() {
		...
		Iterator<Ontology> ontologyIterator = getOntologyModel().listOntologies();
	...
	public List<URI> getImportedOntologyURIs() {
		...
		Iterator<String> uriIterator = getOntologyModel().listImportedOntologyURIs().iterator();
	...
	public List<Individual> getInstances(OntClass ontClass) {
		List<Individual> resultList = new ArrayList<Individual>();
		Iterator<? extends OntResource> resourceIterator = ontClass.listInstances();
	 ...
	 */
	public ObjectProperty addObjectProperty(URI domainClassURI, URI objectPropertyURI, URI rangeClassURI) {
		if (getObjectProperty(objectPropertyURI) == null) {
			if (!containsClass(domainClassURI)) {
				System.out.println("The domainClass " + domainClassURI + " is missing in the ontology. Can't create new object property");
				return null;
			}

			if (!containsClass(rangeClassURI)) {
				System.out.println("The rangeClass " + rangeClassURI + " is missing in the ontology. Can't create new object property");
				return null;
			}
			...
			ObjectProperty newObjectProperty = ontologyModel.createObjectProperty(objectPropertyURI.toString());
			newObjectProperty.setDomain(domainClass);
			newObjectProperty.setRange(rangeClass);
			//MSH: Never used! ObjectProperty testObjectProperty = ontologyModel.getObjectProperty(objectPropertyURI.toString());

			return newObjectProperty;

		} else {
			System.out.println("The object property " + objectPropertyURI + " already existed. New one not created");
			return null;
		}
	 ...
	 */
	public void addDatatypeProperty(URI domainClassURI, URI datatypePropertyURI, Resource datatypeResource) {
		if (getObjectProperty(datatypePropertyURI) == null) {
			if (!containsClass(domainClassURI)) {
				System.out.println("The domain class " + domainClassURI + " is missing in the ontology. Can't create new datatype property");
				return;
			}
			...
			DatatypeProperty newDatatypeProperty = ontologyModel.createDatatypeProperty(datatypePropertyURI.toString());
			...
			System.out.println("The datatype property " + datatypePropertyURI + " already existed. New one not created");
		}
	}
	...
	public <R extends Resource> List<R> getResourcesByClass(Class<R> clazz) {
		...
		if (clazz.equals(OntClass.class)) {
			resourcesInModel = (List<R>)ontologyModel.listClasses().filterDrop(new AnonymousOntClassFilter()).toList();

		}

		else if (clazz.equals(OntProperty.class)) {
			resourcesInModel = (List<R>)ontologyModel.listAllOntProperties().toList();
		}

		else if (clazz.equals(Individual.class)) {
			resourcesInModel = (List<R>)ontologyModel.listIndividuals().toList();
		} else if (clazz.equals(ObjectProperty.class)) {
			resourcesInModel = (List<R>)ontologyModel.listObjectProperties().toList();
		} else if (clazz.equals(DatatypeProperty.class)) {
			resourcesInModel = (List<R>)ontologyModel.listDatatypeProperties().toList();
		}
	 ...
	 * @return a list of Resources having the given Class.
	 */
	public <R extends Resource> List<R> getResourcesByClass(Class<R> clazz, Ontology ontology) {

		return OntologyHelper.getOntResourcesByClass(clazz, ontology, ontologyModel);
	}
	...
	public List<Individual> getObjectPropertyInstances(Individual sourceIndividual, ObjectProperty objectProperty, Ontology ontology) {
		List<Individual> resultIndividuals = new ArrayList<Individual>();

		//MSH: Never used: Resource propertyValue = sourceIndividual.getPropertyResourceValue(objectProperty);
		List<RDFNode> rdfNodes = sourceIndividual.listPropertyValues(objectProperty).toList();
	...
	public OntClass getReferenceOntClass(OntClass sourceOntClass) {
		...
		List<OntClass> equivalentOntClasses = sourceOntClass.listEquivalentClasses().toList();
		for (OntClass equivalentClass : equivalentOntClasses) {
			if (OntologyHelper.isInOntology(getReferenceOntology(), equivalentClass)) {
	...
	public List<DatatypeProperty> getBaseDatatypeProperties() {
		...
		resultList.addAll(getResourcesByClass(DatatypeProperty.class, getBaseOntology()));
		return resultList;
	}

	public List<ObjectProperty> getBaseObjectProperties() {
		List<ObjectProperty> resultList = new ArrayList<ObjectProperty>();
		resultList.addAll(getResourcesByClass(ObjectProperty.class, getBaseOntology()));
		return resultList;
	}
	...
	public List<OntClass> getTopBaseClasses() {
		...
		OntResource topResource = ontologyModel.getOntResource("http://www.w3.org/2000/01/rdf-schema#Resource");
	...
	public Individual getReferenceIndividual(Individual sourceIndividual) {
		List<Individual> equivalentIndividuals = OntologyHelper.getIndividuals(sourceIndividual.listSameAs().toList());
		for (Individual equivalentIndividual : equivalentIndividuals) {
			if (OntologyHelper.isInOntology(getReferenceOntology(), equivalentIndividual)) {
	...
	public OntResource getReferenceOntResource(OntResource sourceOntResource) {
		if (sourceOntResource.isClass()) {
			...
			return getReferenceDatatypeProperty(sourceOntResource.asDatatypeProperty());
		} else if (sourceOntResource.isObjectProperty()) {
			return getReferenceObjectProperty(sourceOntResource.asObjectProperty());
		}
	...
	public DatatypeProperty getReferenceDatatypeProperty(DatatypeProperty sourceDatatypeProperty) {

		List<DatatypeProperty> equivalentDatatypeProperties = OntologyHelper.getDatatypeProperties(sourceDatatypeProperty.listEquivalentProperties().toList());
		for (DatatypeProperty equivalentDatatypeProperty : equivalentDatatypeProperties) {
			if (OntologyHelper.isInOntology(getReferenceOntology(), equivalentDatatypeProperty)) {
	...
	public ObjectProperty getReferenceObjectProperty(ObjectProperty sourceObjectProperty) {

		List<ObjectProperty> equivalentObjectProperties = OntologyHelper.getObjectProperties(sourceObjectProperty.listEquivalentProperties().toList());
		for (ObjectProperty equivalentObjectProperty : equivalentObjectProperties) {
			if (OntologyHelper.isInOntology(getReferenceOntology(), equivalentObjectProperty)) {
	...
	public OntClass getReferenceOntClass(Individual sourceIndividual) {
		...
		for (OntClass candidateOntClass : sourceIndividual.listOntClasses(true).toList()) {
			if (OntologyHelper.isInOntology(getReferenceOntology(), candidateOntClass))
	...
	public OntClass getBaseClass(Individual sourceIndividual) {
		for (OntClass candidateOntClass : sourceIndividual.listOntClasses(true).toList()) {
			if (OntologyHelper.isInOntology(getBaseOntology(), candidateOntClass))
				return candidateOntClass;
		}
		return null;
	}
	...
	public void addDatatypePropertyInstance(URI domainIndividualURI, URI datatypePropertyURI, Literal literal) {
		...
		if (!getIndividual(domainIndividualURI).hasProperty(datatypeProperty)) {
			getIndividual(domainIndividualURI).addProperty(datatypeProperty, literal);
	...
	@SuppressWarnings("unchecked")
	public <R extends OntResource> List<R> getSubOntResources(Class<R> clazz, Ontology ontology, OntResource superOntResource) {
		...
		for (OntResource ontResource : ontResources) {
			if (ontResource.isClass()) {
				...
				List<OntClass> superClasses = ontClass.listSuperClasses(true).filterDrop(new AnonymousOntClassFilter()).toList();
	...
	public void removeOntology(Ontology sourceOntology) {
		List<OntResource> allOntResources = new ArrayList<OntResource>();
		allOntResources.addAll(getResourcesByClass(OntClass.class, sourceOntology));
		allOntResources.addAll(getResourcesByClass(DatatypeProperty.class, sourceOntology));
		allOntResources.addAll(getResourcesByClass(ObjectProperty.class, sourceOntology));
		allOntResources.addAll(getResourcesByClass(Individual.class, sourceOntology));
