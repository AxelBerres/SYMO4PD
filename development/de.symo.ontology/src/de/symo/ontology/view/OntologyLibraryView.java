...
import de.symo.ontology.ActiveOntologyManager;
 ...
 * @since 22.09.2016
 */
public class OntologyLibraryView {
	...
	public void postConstruct(Composite parent) {
		...
		// load the resource
		resource = resourceLoader.getResource(editingDomain, URI.createURI(System.getProperty("user.name") + "/OntLib.onl")).getResource();

		OntologyLibrary lib = (OntologyLibrary)resource.getContents().get(0);
		lib.eAdapters().add(ActiveOntologyManager.getInstance());
		
		
		TreeFormFactory tableFormFactory = injector.getInstance(TreeFormFactory.class);
		// create the tree-form composite
		treeFormComposite = tableFormFactory.createTreeFormComposite(parent, SWT.NONE);
